...
/** Minimal slice of OutstandingReportResponse.OutstandingItem this service actually reads. */
...
export class CustomerService {
   ...
   * this from two other endpoints rather than a new backend aggregate — both
   * gated on BILLING_MANAGE (a separate permission from CUSTOMER_MANAGE), so a
   * user without it simply gets nulls back instead of a 403 bubbling up.
   *
   ...
   * - outstandingAmount: GET /reports/outstanding returns every OPEN invoice
   *   shop-wide in one call; filtered client-side by customerId and summed.
   *   Preferred over an N+1 fan-out over /business-payments/.../outstanding
   *   per invoice, since this endpoint already aggregates the same data.
   */
